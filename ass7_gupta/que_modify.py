...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print("Usage: python3 api_call.py API_KEY query.txt")
        sys.exit(1)
    ...
    with open(query_file, 'r') as f:
        query = f.read()
        q_paragraph.append(query)  
        q_paragraph.append(" Create a question using above concern which can be answered using Maharishi Ramana teachings which can provide help to me. No irrelevant words should be in the question.Keep it in 30 words and no greeting words should be there. It should be straight to the point")
        q_paragraph.append('\n')
